...
class AiService {
    constructor(http, config, realtimeHttp) {
        ...
        this.databaseId = config?.databaseId; // Added to match PingDartClient mapping
    }
    ...
    async callAiApi(message, onProgress, options = {}) {
        try {
            const payload = {
                ...
                apikey: options.apikey || this.agentKey, // Agent Specific Key
                user_id: options.userId || "default", 
                database_id: options.databaseId || this.databaseId, // Important for auth routing
                ...options
            };
